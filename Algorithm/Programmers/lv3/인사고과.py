...
"""
1. 사람들끼리 비교하면 O(n^2의 복잡도를 가짐) -> 시간초과
2. 완호와 사람들을 비교! 리스트 첫번째 값을 기준으로 내림차순, 두 번째 값을 기준으로 오름차순 정렬!
   리스트에서 현재 점수의 첫번째값은 그 전에 나온 점수의 첫번째값보다 같거나 작다!
   '''
어떤 사원이 다른 임의의 사원보다 두 점수가 모두 낮은 경우가 한 번이라도 있다면 그 사원은 인센티브를 받지 못합니다.
'''
   하지만 두번째 값은 정렬이 덜 된 상태이다! 현재 두번째값이 최대값 변수보다 낮으면 진행시키지 않는다! 걸러지는 조건! 필터링!
"""
