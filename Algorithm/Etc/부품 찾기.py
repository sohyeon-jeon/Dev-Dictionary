...
class Test1:
    def binary_search(components,target,start,end):
        while start<=end:
            mid=(start+end)//2
    ...
    # 이진 탐색을 수행하기 위해 미리 정렬 수행
    components.sort()

    # 손님이 요청한 부품을 하나씩 확인
    for search in search_array:
        stock=binary_search(components,search,0,len(components)-1)
        if stock:
            print('yes',end=' ')
        else:
            print('no',end=' ')
    ...
    """
    다량의 데이터 검색(1000만건 이상)은 이진 탐색 알고리즘을 이용해 효과적으로 처리할 수 있다.
    """
